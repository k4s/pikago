...
// Package ws implements a simple WebSocket transport for pikago.
...
const (
	...
	// pikago Listen() method to start up the server.
	...
	// server is also started with pikago Listen().  This means that you
...
func (w *wsPipe) Recv() (*pikago.Message, error) {
	...
	msg := pikago.NewMessage(0)
	msg.Body = body
	return msg, nil
}

func (w *wsPipe) Send(m *pikago.Message) error {
...
func (d *dialer) Dial() (pikago.Pipe, error) {
	...
	w.props[pikago.PropLocalAddr] = w.ws.LocalAddr()
	w.props[pikago.PropRemoteAddr] = w.ws.RemoteAddr()
...
func (l *listener) Listen() error {
	...
	if taddr, err = pikago.ResolveTCPAddr(l.url.Host); err != nil {
		return err
	}
...
func (l *listener) handler(ws *websocket.Conn, req *http.Request) {
	...
	w.props = make(map[string]interface{})
	w.props[pikago.PropLocalAddr] = ws.LocalAddr()
	w.props[pikago.PropRemoteAddr] = ws.RemoteAddr()

	if req.TLS != nil {
		w.props[pikago.PropTLSConnState] = *req.TLS
	}
...
func (wsTran) NewDialer(addr string, sock pikago.Socket) (pikago.PipeDialer, error) {
	iswss := strings.HasPrefix(addr, "wss://")
	opts := make(map[string]interface{})

	opts[pikago.OptionNoDelay] = true
	opts[pikago.OptionKeepAlive] = true
	proto := sock.GetProtocol()
	maxrx := 0
	if v, e := sock.GetOption(pikago.OptionMaxRecvSize); e == nil {
		maxrx = v.(int)
	}
...
func (t wsTran) NewListener(addr string, sock pikago.Socket) (pikago.PipeListener, error) {
	proto := sock.GetProtocol()
...
func (wsTran) listener(addr string, proto pikago.Protocol) (*listener, error) {
	var err error
